...
class GeolocationParams:
    ...
    def setIPAddresses(self, ipAddresses=[]):
        if len(ipAddresses) > 50:
            ValueError(
                "Maximum number of IP addresses for bulk lookup cannot be more than 50."
            )
        else:
...
class IPGeolocationAPI:
    ...
    def __get(self, path, urlParams={}):
        ...
        try:
            ...
            if response.status_code != 200:
                print(
                    "HTTP Get Request Failed: status_code={0}, response={1}".format(
    ...
    def __post(self, path, requestData="", urlParams={}):
        ...
        try:
            ...
            if response.status_code != 200:
                print(
                    "HTTP Post Request Failed: status_code={0}, response={1}".format(
                        response.status_code, jsonResponse
                    )
                )
        except RequestException as e:
